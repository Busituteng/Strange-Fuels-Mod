...
public class BiomeHuangdanzhidiqunxi extends ElementsStrangeFuelsMod.ModElement {
	@GameRegistry.ObjectHolder("strangefuelsmod:huangdanzhidiqunxi")
	public static final BiomeGenCustom biome = null;
	public BiomeHuangdanzhidiqunxi(ElementsStrangeFuelsMod instance) {
		super(instance, 306);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Absurdland").setRainfall(0.2F).setBaseHeight(0.15F).setWaterColor(-65536).setHeightVariation(0.25F)
					.setTemperature(0.7F));
			setRegistryName("huangdanzhidiqunxi");
			topBlock = Blocks.EMERALD_BLOCK.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.EMERALD_BLOCK.getDefaultState().getBlock()
							|| ground == Blocks.REDSTONE_BLOCK.getDefaultState().getBlock())
					...
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), Blocks.REDSTONE_BLOCK.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == Blocks.GOLD_BLOCK.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, Blocks.GOLD_BLOCK.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.GOLD_BLOCK.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), Blocks.LAPIS_BLOCK.getDefaultState());
							}
						}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.LAPIS_BLOCK.getDefaultState().getBlock()
					|| blockType == Blocks.GOLD_BLOCK.getDefaultState().getBlock() || blockType == Blocks.EMERALD_BLOCK.getDefaultState().getBlock()
